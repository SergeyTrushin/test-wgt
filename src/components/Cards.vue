  ...
  <div id="main">
    <div class="items" ref="items">
      ...
      <div v-else class="item" v-for="curName of paginatedData">
        <p class="curName">
          {{ value ? value : 0 }} <span>{{ baseCurrency }} = </span>
        </p>
        <div class="curValue">
          <span class="valueOfCur">
            {{
              calc(getRates[curName]).replace(/\./g, ",") !== "NaN"
                ? calc(getRates[curName]).replace(/\./g, ",")
                : "0"
            }}
          </span>
          <span class="curName">
    ...
    <div class="btn">
      <button class="back" @click="prevPage" :disabled="pageNumber == 0">
        Назад
      </button>
      <button
        class="next"
