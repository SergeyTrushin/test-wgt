<template>
  <div id="app" v-responsive="{ small: (el) => el.width < 720 }">
    <div id="container" ref="con">
      ...
      <div class="input">
        <input type="tel" @input="clear" v-model:value="value" />
        <span>{{ baseCurrency }} </span>
      </div>
      <Cards
        v-responsive="{ small: (el) => el.width < 720 }"
        :value="value"
        :size="size"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    next() {
      if (
        this.paginatedData[this.paginatedData.length - 1] ===
        this.getListOfRates[this.activeCur]
      ) {
        this.pageNumber++;
      }
